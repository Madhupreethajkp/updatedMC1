...
class PolicyRegister(Resource):
    ...
    def post(self):
        ...
        num = "001"
        get_plist = PolicyList.get(self)
        pol_list = get_plist['policies']
        polid = ""
        for iterator in pol_list:
            polid = iterator['policy_id']
        lastid = (polid.split('-')[2])
        num = int(lastid) + 1
        final = "{0:0=3d}".format(num)
        print(num)
        ...
        year_of_start_date = dt.year
        print(type(year_of_start_date))
        policy_id = policy_type_id + '-' + str(year_of_start_date) + '-' + final
